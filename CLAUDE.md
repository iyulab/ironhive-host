...
ironhive-cli는 **지시된 단일 임무를 철저히 수행하는 에이전트 코어**입니다.

### 단일 책임 원칙

| 수행 | 비수행 |
|------|--------|
| 명령 수신 → 계획 수립 → 임무 수행 | 무중단 자동 재개 |
| 세션 상태 관리 및 복원 | 칸반 보드, 작업 큐 |
| 플러그인 기반 도구 확장 | 복잡한 멀티에이전트 오케스트레이션 |

**확장성 원칙**: 복잡한 워크플로우(자동 재시도, 스케줄링, 배치 처리 등)는 이 CLI를 호출하는 상위 계층 프로그램이나 웹훅 통합을 통해 구현합니다.

### 핵심 역량

1. **세션 지속성**: 작업 상태를 정확히 기억하고, 중단된 지점에서 재개
2. **플러그인 호환성**: Claude Code 플러그인 시스템과 100% 호환
3. **컨텍스트 관리**: 효율적인 토큰 사용과 자동 압축
...
| **토큰 관리** | IndexThinking | `IndexThinking 0.15.0` |
