...
| ID | 태스크 | 설명 | 의존성 | 참조 |
|----|--------|------|--------|------|
| P5a-01 | ~~웹훅 시스템~~ | ✅ IWebhookService, WebhookService (HMAC 서명, 재시도, 비동기 전송) | P2-01 | - |
| P5a-02 | ~~설정 파일 체계~~ | ✅ ConfigurationManager (계층적 병합: Global→Project→Env→.env) | P3-06 | [ref.md#4](../dev-docs/ref.md) |
| P5a-03 | ~~CLAUDE.md 지원~~ | ✅ LoadClaudeMd, GetMergedClaudeMdContent (부모 디렉토리 상속) | P5a-02 | [github-ref](./coding-agent-github-ref.md) |
| P5a-04 | ~~에러 복구 고도화~~ | ✅ ErrorRecoveryService (카테고리별 복구, 패턴 감지, 에스컬레이션) | P2-10 | [research-01#5.2](./research/research-01.md), [research-02#6.2](./research/research-02.md) |
| P5a-05 | ~~비용/토큰 제한~~ | ✅ UsageLimiter (세션 한도, 경고 임계치, 자동 중지) | P4-02 | [ref.md#5](../dev-docs/ref.md) |
| P5a-06 | ~~크로스 플랫폼 테스트~~ | ✅ CliE2ETests (Process 기반 실제 CLI 실행) | P0-05 | - |
| P5a-07 | ~~🧪 E2E 테스트 스위트~~ | ✅ 12개 E2E 시나리오 (help, version, config, sessions 등) | P5a-06 | - |
| P5a-08 | ~~🧪 성능 벤치마크~~ | ✅ 14개 벤치마크 (토큰 카운팅, 압축, 에러 복구, 웹훅 등) | P5a-07 | - |

### 산출물
- ✅ **WebhookService**: 이벤트 전송, HMAC 서명, 이벤트 필터링
- ✅ **ConfigurationManager**: 계층적 설정 병합, 환경 변수 우선
- ✅ **CLAUDE.md 지원**: 프로젝트별 지시사항 자동 로드
- ✅ **ErrorRecoveryService**: 에러 카테고리화, 복구 추천, 에스컬레이션
- ✅ **UsageLimiter**: 토큰/비용 제한, 경고 임계치
- ✅ **E2E 테스트 스위트**: 12개 시나리오
- ✅ **성능 벤치마크**: 14개 벤치마크 테스트

### 진행 상황
- **Phase 5a 완료**: P5a-01~P5a-08 (8/8 태스크, 100%) ✅
- **테스트 현황**: 512개 테스트 (506 통과, 6 E2E 환경 의존)
  - WebhookServiceTests: 13 tests
  - ConfigurationManagerTests: 14 tests
  - ErrorRecoveryServiceTests: 20 tests
  - UsageLimiterTests: 17 tests
  - CliE2ETests: 12 tests
  - PerformanceBenchmarks: 14 tests
- **Phase 5b로 진행 가능**
...
| P5b-02 | ~~🧪 실제 LLM 통합 테스트~~ | ✅ GpuStack 통합 테스트 (환경 변수 조건부 실행) | P5a-07 | - |
| P5b-03 | ~~🧪 회귀 테스트 자동화~~ | ✅ CI에서 크로스 플랫폼 테스트 (Ubuntu/Windows/macOS) | P5a-07, P0-05 | - |
| P5b-04 | ~~문서화~~ | ✅ README.md 업데이트 (설치, 사용법, 설정, 아키텍처) | - | - |
...
v0.5.0 ─── Phase 4: 컨텍스트 관리 + 프롬프트 캐싱       ✅ 완료 (100%)
   ...
   │
v0.6.0 ─── Phase 5a: 안정화 + E2E 테스트               ✅ 완료 (100%)
   │       └── P5a-01~P5a-08 완료 (8/8)
   │       └── WebhookService, ConfigurationManager, ErrorRecoveryService
   │       └── UsageLimiter, E2E Tests (12개), Benchmarks (14개)
   │       └── 512 tests (506 통과)
   │
v0.7.0 ─── Phase 5b: 생태계 통합 + 릴리스 자동화        ✅ 완료 (100%)
   │       └── P5b-01~P5b-06 완료 (6/6)
